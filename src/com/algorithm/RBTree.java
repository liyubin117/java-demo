...
//		return search2(root, key); //使用递归的方法，本质一样的
	 ...
	 *   ly ry               lx ly
	 * 左旋做了三件事：
	 * 1. 将y的左子节点ly赋给x的右子节点,并将x赋给y左子节点的父节点(y左子节点非空时)
	 * 2. 将x的父节点p(非空时)赋给y的父节点，同时更新p的子节点为y(左或右)
	 * 3. 将y的左子节点设为x，将x的父节点设为y
	 */
	private void leftRotate(RBNode<T> x) {
		//1. 将y的左子节点赋给x的右子节点，并将x赋给y左子节点的父节点(y左子节点非空时)
		//（其实是一组操作，将一个节点a赋为另一节点b的子节点，b在实际意义上也就是a的父节点，但仍需赋值才能体现）
//		  x  
//		  / \
//		 lx ly
		RBNode<T> y = x.right;
...
//		 lx ly
	...
	//将节点插入到红黑树中，这个过程与二叉搜索树是一样的，只是多了修正为红黑树的过程
	...
	private void insertFixUp(RBNode<T> node) {
		...
		//需要修整的条件：父节点存在，且父节点的颜色是红色（说明父节点肯定不是根节点，即父节点肯定是有祖父节点的）。插入后违反红黑相间规则。
